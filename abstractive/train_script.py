...
if __name__ == '__main__':
    ...
    if not test:
        ...
        #filename_test = 'test_setup2.pkl'
        test_filename = 'testout/test.txt'
        gold_truth = 'testout/gold.txt'
    if test:
        preprocess = preprocess4model.Preprocessing()
        in_tk2id, in_id2tk, out_tk2id, out_id2tk, input_list, output_list = preprocess.train_preprocess_mini(intokennum=5000, outtokennum=5000)
        validation_input_list, validation_output_list = preprocess.test_preprocess_all(filename='dvp_nyt.pkl', numofdata=20)
        batch_size = 2
        learning_rate = 0.01
        init_accum = 0.
        steps = 20
    ...
    if train_label:
        input, input_lengths = encoding(input_list, in_tk2id, max_length=400)
        summary, summary_lengths = encoding(output_list, out_tk2id, max_length=100)
        vld_input, vld_input_lengths = encoding(validation_input_list, in_tk2id, max_length=400)
        vld_summary, vld_summary_lengths = encoding(validation_output_list, in_tk2id, max_length=100)
        print('Data prepared')
        n_iters = 0
        for i in range(n_iters):
            if epoch < 2:
                print('Epoch: ', epoch)
                trainIters(encoder1, attn_decoder1, encoder_optimizer, decoder_optimizer, steps, batch_size, input_1, input_lengths_1, summary_1, summary_lengths_1,
                           print_every=print_every, device=device, max_target_length=50)
                ...
                ckpid = epoch % 10
                checkpoint_name = ckp_path + checkpoint_name_base + '_%d.tar' %ckpid
                torch.save({
                ...
                if epoch == 2:
                    # encoder_optimizer = optim.Adagrad(encoder1.parameters(), lr=learning_rate,
                    #                                   initial_accumulator_value=init_accum)
                    # decoder_optimizer = optim.Adagrad(attn_decoder1.parameters(), lr=learning_rate,
                    #                                   initial_accumulator_value=init_accum)
                    print('Optimizer Initialized')
                print('Epoch: ', epoch)
                trainIters(encoder1, attn_decoder1, encoder_optimizer, decoder_optimizer, steps, batch_size, input_2, input_lengths_2, summary_1,
                           summary_lengths_1,
                           print_every=print_every, device=device, max_target_length=50)
                validation(encoder1, attn_decoder1, encoder_optimizer, decoder_optimizer, steps, batch_size,
                           vld_input, vld_input_lengths, vld_summary, vld_summary_lengths,
    ...
    if test:
        ...
        for i in range(10):
            ...
            prediction = beam_evaluate_unk(encoder1, attn_decoder1, sent, input_length, out_lang, max_length=MAX_LENGTH, beam_size=3, min_target_length=0, block_ngram_repeat=0, )
            print(' '.join(prediction))
            print(' '.join(true))
    if not test:
        test_input, test_input_lengths = encoding(test_input_list, in_tk2id, max_length=400)
        ...
        for i in range(length):
            ...
            #prediction,_ = evaluate(encoder1, attn_decoder1,  sent, input_length, out_lang, max_length=MAX_LENGTH)
            prediction = beam_evaluate_unk(encoder1, attn_decoder1, sent, input_length, out_lang, max_length=MAX_LENGTH, beam_size=5, min_target_length=20,
                                           printnum=1, block_ngram_repeat=3, max_target_length=100, pre_input=pre_input)
