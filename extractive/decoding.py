...
class DecodeDatasetEntity(CnnDmDataset):
    ...
    def __getitem__(self, i):
        js_data = super().__getitem__(i)
        art_sents, clusters  = js_data['article'], js_data[self.key]
        return art_sents, clusters
...
class ExtractorEntity(object):
    def __init__(self, ext_dir, max_ext=5, cuda=True):
        ext_meta = json.load(open(join(ext_dir, 'meta.json')))
        if ext_meta['net'] == 'ml_entity_extractor':
            ext_cls = PtrExtractSummEntity
        elif ext_meta['net_args']['extractor']['net'] == 'ml_entity_extractor':
            ext_cls = PtrExtractSummEntity
        else:
            raise ValueError()
        ext_ckpt = load_best_ckpt(ext_dir)
    ...
    def __call__(self, raw_article_sents, raw_clusters):
        ...
        clusters = (conver2id(UNK, self._word2id, raw_clusters[0]), raw_clusters[1], raw_clusters[2])
        article = pad_batch_tensorize(articles, PAD, cuda=False, max_num=5
                                     ).to(self._device)
        clusters = (pad_batch_tensorize(clusters[0], PAD, cuda=False, max_num=4
                                      ).to(self._device),
                    pad_batch_tensorize(clusters[1], PAD, cuda=False, max_num=4
                                        ).to(self._device),
                    pad_batch_tensorize(clusters[2], PAD, cuda=False, max_num=4
                                        ).to(self._device))
        if raw_clusters == []:
            print(clusters)

        indices = self._net.extract([article], clusters, k=min(n_art, self._max_ext))
        return indices
...
class SCExtractor(object):
    def __init__(self, ext_dir, cuda=True, entity=False):
        ext_meta = json.load(open(join(ext_dir, 'meta.json')))
        assert ext_meta['net'] == 'rnn-ext_abs_rl'
        ext_args = ext_meta['net_args']['extractor']['net_args']
        word2id = pkl.load(open(join(ext_dir, 'agent_vocab.pkl'), 'rb'))
        if entity:
            extractor = PtrExtractSummEntity(**ext_args)
    ...
    def __call__(self, raw_article_sents):
        self._net.eval()
        indices, _, _ = self._net(raw_article_sents)
        return indices