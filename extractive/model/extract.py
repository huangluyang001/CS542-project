...
class PtrExtractSumm(nn.Module):
    ...
    def __init__(self, emb_dim, vocab_size, conv_hidden,
                 ...
                 n_hop=1, dropout=0.0, pe=False, petrainable=False, stop=False):
        super().__init__()
        if petrainable == True:
            embedding = nn.Embedding(512, emb_dim, padding_idx=0)
        else:
            embedding = None
        self._sent_enc = ConvSentEncoder(
            vocab_size, emb_dim, conv_hidden, dropout, pe, petrainable=petrainable, embedding=embedding)
        self._art_enc = LSTMEncoder(
            3*conv_hidden, lstm_hidden, lstm_layer,
            dropout=dropout, bidirectional=bidirectional
        )
...
class PtrExtractSummEntity(nn.Module):
    ...
    def __init__(self, emb_dim, vocab_size, conv_hidden,
                 lstm_hidden, lstm_layer, bidirectional,
                 n_hop=1, dropout=0.0, pe=False, petrainable=False, stop=False, hard_attention=False, context=False, rnn_entity=False):
        super().__init__()
        enc_out_dim = lstm_hidden * (2 if bidirectional else 1)

        self._embedding = nn.Embedding(vocab_size, emb_dim, padding_idx=0)
        self._sent_enc = ConvSentEncoder(
            vocab_size, emb_dim, conv_hidden, dropout, pe, embedding=self._embedding)
        self._entity_enc = ConvEntityEncoder(
            vocab_size, emb_dim, conv_hidden, dropout, pe, embedding=self._embedding,
            context=context, context_hidden=emb_dim, context_input_dim=enc_out_dim
        )
        if rnn_entity:
            self._mention_cluster_enc = LSTMEncoder(
            3*conv_hidden, lstm_hidden, lstm_layer,
            dropout=dropout, bidirectional=bidirectional
        )
        ...
        if rnn_entity:
            ...
            enc_out_dim, lstm_hidden, lstm_layer,
            dropout, n_hop, enc_out_dim, stop, hard_attention
        )
        else:
            self._extractor = LSTMPointerNet_entity(
                enc_out_dim, lstm_hidden, lstm_layer,
                dropout, n_hop, 3*conv_hidden, stop, hard_attention
            )

        self._context = context
        self._rnn_entity = rnn_entity
    ...
    def forward(self, article_sents, sent_nums, target, clusters, cluster_nums):
        ...
        if not self._extractor._hard_attention:
            output = self._extractor(enc_out, sent_nums, ptr_in, entity_out, cluster_nums)
            return output
        else:
            output, selection = self._extractor(enc_out, sent_nums, ptr_in, entity_out, cluster_nums, clusters[3])
            return (output, selection)
    ...
    def _encode_entity(self, clusters, cluster_nums, context=None):
        ...
        if self._rnn_entity:
            if cluster_nums is not None:
                enc_entity = self._mention_cluster_enc(enc_entity, cluster_nums)
            else:
