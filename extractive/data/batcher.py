...
def prepro_fn_extract_hardattn(max_src_len, max_src_num, batch, pad=0, split_token='<split>'):
    # split will be "split token"
    def prepro_one(sample):
        source_sents, extracts, clusters, labels = sample
        tokenized_sents = tokenize(max_src_len, source_sents)[:max_src_num]
        cleaned_labels = []
        cleaned_extracts = []
        for label, extract in zip(labels, extracts):
            if extract < len(tokenized_sents):
                cleaned_labels.append(label)
                cleaned_extracts.append(extract)
        #cleaned_extracts = list(filter(lambda e: e < len(tokenized_sents),
        #                               extracts))
        # merge cluster
        cluster_words = []
        ...
        for cluster in clusters:
            ...
            scluster_spos = []
            for mention in cluster:
                if mention['position'][0] > max_src_num-1:
                    continue
                if len(mention['text'].strip().split(' ')) == len(
                        list(range(mention['position'][3] + 1, mention['position'][4] + 1))):
                    scluster_word += mention['text'].lower().strip().split(' ')
        ...
        for id, label in enumerate(labels):
            for ind, lb in enumerate(label):
                if lb > len(cluster_words):
                    labels[id][ind] = len(cluster_words) # force model to attend pad entity


        return tokenized_sents, cleaned_extracts, clusters, cleaned_labels
    batch = list(map(prepro_one, batch))
    return batch
